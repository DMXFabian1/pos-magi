...
class ApartadosLogic:
    ...
    def agregar_cliente(self, nombre_completo, numero):
        try:
            if nombre_completo is None or numero is None:
                raise ValueError("El nombre completo y el número no pueden ser None")
            cursor = self.db_manager.get_cursor()
            cursor.execute("""
                INSERT INTO clientes (nombre_completo, numero, store_id)
                VALUES (?, ?, ?)
            """, (nombre_completo, numero, self.store_id))
    ...
    def editar_cliente(self, cliente_id, nombre_completo, numero):
        try:
            ...
            cursor = self.db_manager.get_cursor()
            cursor.execute("""
                UPDATE clientes SET nombre_completo = ?, numero = ?
                WHERE id = ? AND store_id = ?
            """, (nombre_completo, numero, cliente_id, self.store_id))
    ...
    def eliminar_cliente(self, cliente_id):
        try:
            ...
            cursor.execute("DELETE FROM clientes WHERE id = ? AND store_id = ?", (cliente_id, self.store_id))
            if cursor.rowcount == 0:
                raise ValueError("Cliente no encontrado")
            self.db_manager.conn.commit()
