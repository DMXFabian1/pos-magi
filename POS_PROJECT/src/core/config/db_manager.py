...
class DatabaseManager:
        ...
        try:
            ...
            if not cursor.fetchone():
                ...
                for col in expected_columns:
                    if col not in columns and col != "store_id":
    ...
    def validar_stock(self, sku, cantidad, store_id=1):
        try:
            ...
            logger.error(f"Error al validar stock para SKU {sku} en tienda {store_id}: {str(e)}")
            return False
    ...
    def registrar_venta(self, user_id, items, metodo_pago, id_cliente, tasa_iva, store_id=1, reducir_inventario=True):
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (user_id, id_cliente, metodo_pago, fecha, total, json.dumps(items), store_id))
    ...
    def completar_apartado(self, apartado_id, metodo_pago, tasa_iva, store_id=1):
        try:
            ...
            apartado = cursor.fetchone()
            if not apartado:
                raise ValueError(f"No se encontró el apartado con ID {apartado_id} в tienda {store_id}")
